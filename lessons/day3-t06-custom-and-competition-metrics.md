...
3. **Is there a floor, cap or threshold?** A `min(1, ·)` caps per-example damage; a rescaling floor can turn a mediocre score into exactly zero.
...
2. `min(1, ·)` caps the damage, so a catastrophic failure on one image is bounded. Never skip an image — output a plausible number. Predicting the training mean beats predicting nothing.
...
1. `|132−120|/120 = 0.1`, so accuracy `0.9`.
2. Error rate `1.0`, accuracy `1 − min(1, 1) = 0`. The `min(1, ·)` floors each image at zero, so one disaster cannot drag the mean below the others.
...
**Rep:** implement `task6_score` and test it on synthetic counts drawn uniformly from 100–500 under three error models — constant bias +5%, multiplicative noise ±10%, and one catastrophic miss in fifty — and rank which error model costs most.
...
- `min(1, ·)` bounds per-example damage, so always emit a guess, never a blank.
