...
**Why "gradient".** Squared error makes the residual $y - F$ equal to the negative gradient of the loss with respect to the prediction. Swap in a different loss and you fit the negative gradient instead — that is all "gradient boosting" means. For binary classification the loss is log-loss, the model predicts a **log-odds** score $F(x)$, the probability is $\sigma(F(x)) = \frac{1}{1 + e^{-F(x)}}$, and the "residual" each tree fits is $y_i - \sigma(F(x_i))$ — the probability error. Multi-class (42 letters in `qaz_letters.md`) fits one additive model per class and softmaxes at the end.
...
**Learning rate vs n_estimators.** $\nu$ (`learning_rate`) scales each tree's contribution, $M$ (`n_estimators`/`max_iter`) is how many you add, and their product is the capacity — so halve $\nu$ and roughly double $M$. Small $\nu$ with large $M$ generalises better and costs more time; $\nu \in [0.05, 0.1]$ is the working range, $\nu = 0.3$ is for when the clock binds.
...
Then [`qaz_letters.md`](../faio-2025/day1/qaz_letters.md): 42 classes, so boosting fits 42 additive models and wall-clock is $42\times$ a binary problem — lower `max_iter` and keep `early_stopping=True`. And for task 4's sparse 20,000-column TF-IDF matrix boosting is the *wrong* first model: linear models (T08) dominate high-dimensional sparse text, which is why the notebook ships logistic regression and lists boosting only as an ensemble partner.
...
1. A small tree fitted to the negative gradient of the loss at the current predictions (for squared error, the residual $y - F$); `learning_rate` $\times$ that tree is added to the running prediction.
2. Roughly double it. Capacity is governed by $\nu \cdot M$, so smaller steps need more of them to reach the same fit — which is why you set `max_iter` high and let early stopping find the real value.
...
4. A log-odds score; $p = \sigma(F(x)) = \frac{1}{1 + e^{-F(x)}}$.
...
- `learning_rate` $\times$ `n_estimators` is the capacity; tune one and let early stopping set the other — without it, `max_iter` is a guess that overfits.
...
- Multi-class costs one additive model per class: 42 classes in `qaz_letters.md` is $42\times$ the fits.
