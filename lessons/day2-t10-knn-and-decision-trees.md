...
Distance metrics: Euclidean `√Σ(aᵢ−bᵢ)²` (`p=2`), Manhattan `Σ|aᵢ−bᵢ|` (`p=1`), Minkowski with general `p`, cosine `1 − a·b/(‖a‖‖b‖)` when only direction matters (text, embeddings). `weights="distance"` lets nearer neighbours vote harder.
...
**Decision tree — a learned sequence of questions.** At each node, choose the feature and threshold whose split makes the children purest. Impurity of a node with class shares `pₖ`:

```
Gini    = 1 − Σ pₖ²            (max 0.5 for two balanced classes)
Entropy = − Σ pₖ log₂ pₖ       (max 1 bit for two balanced classes)
```

The split maximises the weighted impurity drop, `I(parent) − Σ (nᵢ/n)·I(childᵢ)`. Gini and entropy almost never disagree on the final tree; Gini is cheaper and is the default.
...
3. `p = 0.75`, `1 − (0.75² + 0.25²) = 1 − (0.5625 + 0.0625) = 0.375`.
