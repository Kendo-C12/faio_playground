...
**Definition.** For a discrete random variable $X$ taking value $x_i$ with probability $p_i$:
...
**Indicators.** Let $\mathbf{1}_A$ be 1 when event $A$ happens and 0 otherwise. Then
...
**Worked example — the HearMe score.** [`HearMe_Personalized_Music_Recommender.md`](../faio-2025/day2/HearMe_Personalized_Music_Recommender.md) defines, for one user,

$$
S_u = \sum_{j=1}^{50} f_j
$$

where $f_j \in \{0, 0.25, 0.5, 0.75, 1.0\}$ is the fraction of recommended track $j$ the user actually listened to. By linearity,

$$
E[S_u] = \sum_{j=1}^{50} E[f_j]
$$

So each of the 50 slots contributes independently-in-expectation, and the best possible slate is simply *the 50 tracks with the highest $E[f_j]$*. Each $E[f_j]$ is itself an expectation over the five-point scale:

$$
E[f] = 0 \cdot P(0) + 0.25 \cdot P(0.25) + 0.5 \cdot P(0.5) + 0.75 \cdot P(0.75) + 1 \cdot P(1)
$$

Call that the track's **expected listening depth** for this user. A track with $P(1.0) = 0.3$ and $P(0) = 0.7$ scores $E[f] = 0.30$. A track with $P(0.25) = 0.9$, $P(0) = 0.1$ scores $0.225$ — less, despite being listened to nine times out of ten. Probability of engagement is not the objective; depth $\times$ probability is.

**Worked example — Hit@3.** [`Lost_in_the_Museum.md`](../faio-2025/day2/Lost_in_the_Museum.md) scores

$$
\text{Hit@3} = \frac{\text{number of queries with correct match in Top-3}}{1000}
$$

That is the mean of 1000 indicators, so it estimates $P(\text{true match lands in the top 3})$ for a random query. Nothing else about the ranking is in the metric: rank 1 and rank 3 are worth the same, rank 4 and rank 19,000 are worth the same.
...
2. "Average per user… then normalized score between 0 and 1" → the final number is $E[S_u]/50$ over a random user, i.e. the **mean expected listening depth per recommended track**. The statement's own reading confirms the scale: "~0.5 → Users listen to about half of your recommendations."
...
4. "no cold-start users" → every test user has history, so every $E[f_j]$ is estimable from that user's own behaviour. There is no excuse for falling back to a global prior.

Now the payoff question: **why does ranking by expected listening depth beat ranking by popularity?** Popularity ranks by $P(\text{anyone interacts})$, marginalised over all users. The metric pays $E[f \mid \text{this user}]$. A globally popular track the user has already heard to death, or in a genre they skip, has high popularity and low $E[f]$ for them — you spend a slot worth ~0.1 where a personalised pick was worth ~0.6. Across 50 slots and a normalised 0–1 scale, that is the whole gap between "0.25, users are starting to engage" and "~0.5, clearly interesting". Popularity is only the right answer when you have no per-user signal, which this task explicitly denies you.
...
1. $0 \cdot 0.5 + 0.5 \cdot 0.3 + 1 \cdot 0.2 = 0.35$.
2. $E[S] = 50 \times 0.4 = 20$ out of 50; normalised $20/50 = 0.4$.
3. B: $E[f_B] = 0.5 \cdot 0.55 = 0.275$ versus $E[f_A] = 1 \cdot 0.25 = 0.25$. Depth alone does not decide it — the product does.
4. Linearity of expectation needs no independence: $E\!\left[\sum_j f_j\right] = \sum_j E[f_j]$ regardless of how the $f_j$ are correlated. The sum is what the grader computes, so maximising the sum of expectations is exactly right.
...
6. $\text{Hit@3} = E\big[\mathbf{1}\{\text{true match} \in \text{Top3}(\mathbf{q})\}\big] = P(\text{true match} \in \text{Top3}(\mathbf{q}))$, estimated by the mean over the 1000 queries.

</details>

**Rep:** for the HearMe metric, write down the expected score of a submission that fills all 50 slots with the globally most-played tracks, given $E[f] \approx 0.12$ for an unpersonalised pick, and compare with $E[f] \approx 0.45$ for a personalised one. That gap is the whole task.

## Traps & 60-second recall

- $E[X] = \sum_i x_i p_i$. $E[\mathbf{1}_A] = P(A)$. $E\!\left[\sum_i X_i\right] = \sum_i E[X_i]$, dependence allowed.
- A metric that sums per-item scores is optimised slot by slot — rank by the per-item expectation.
- Probability of engagement $\ne$ expected depth. Multiply, do not substitute.
